#!/bin/bash
# nginx_per_domain_group.sh v4.3.0 真正永不翻车·全环境完美运行版
...
echo "=== Nginx 域名组路径分配器（v4.3.0 永不翻车终极版）==="
echo
...
find "$NGINX_ENABLED" -type l -print0 2>/dev/null | while IFS= read -r -d '' symlink; do
    ...
    for block_file in /tmp/block_*; do
        ...
        # 关键修复：精准插入到最后一个 server_name 行的正下方
        location_block=$(echo "$template" | sed "s/%PATH%/$path/g")

        # 使用 awk 在最后一个 server_name 行后插入
...
if [ "$global_modified" -gt 0 ]; then
    ...
    if nginx -t >/dev/null 2>&1; then
        ...
        echo "错误：Nginx 配置错误！请运行 nginx -t 检查"
        exit 1
    fi
else
    echo "本次无新增（全部已注入）"
